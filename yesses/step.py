    ...
    def check_has_keys(self, _):
        if self.required_keys is None:
            ...
            if not isinstance(self.data, list):
                raise Exception(f"Expected input to be a list, but it is {type(self.data)}")
            required_keys = set(self.required_keys)
            for element in self.data:
                if not isinstance(element, dict):
                    raise Exception(f"Expected input to be a dictionary, but it is {type(element)}")
                provided_keys = set(element.keys())
                if not required_keys.issubset(provided_keys):
                    missing_keys = required_keys - provided_keys
                    raise Exception(f"Provided input is missing the key(s): {', '.join(missing_keys)}")
    ...
    def check_has_keys(self, provided_keys_in_global_findingslist):
        ...
        for key in self.findingskeys:
            if not key in provided_keys_in_global_findingslist:
                raise Exception(f"No input with name '{key}' exists. Valid input keys existing at that point in the run: {', '.join(provided_keys_in_global_findingslist.keys())}")
            provided_keys = set(provided_keys_in_global_findingslist[key])
            if not required_keys.issubset(provided_keys):
                missing_keys = required_keys - provided_keys
                raise Exception(f"Input is missing the key(s): {', '.join(missing_keys)}")
...
class Step:
    ...
    def parse_find(self):
        ...
        for name, alias in FindParser.parse_find_mapping(self.raw['find']).items():
            output_field, properties = self.action_class.find_matching_output_field(name)
            self.outputs.append(StepOutput(name=name, alias=alias, provided_keys=properties['provided_keys']))
    ...
    def parse_expect(self):
        ...
        for function in functions:
            self.required_fields_for_expect.append((function.rule, function.required_fields))
        ...
        for rule, required_fields in self.required_fields_for_expect:
            for required_field in required_fields:
                ...
                for output in self.outputs:
                    ...
                    raise Exception(f"Rule '{rule}' requires output '{required_field}', which does not exist at this point in the run.")
    ...
    def parse_inputs(self):
        ...
        for name, value in self.kwargs.items():
            ...
            if type(value) is not str or not value.startswith('use '):
                self.inputs[name] = LiteralStepInput(data=value, required_keys=required_keys)
                continue
            
            keys = [group.key for group in UseParser.parse(value)]
            self.inputs[name] = GlobalFindingsStepInput(findingskeys=keys, required_keys=required_keys)
    ...
    def load_findings(self, findings):
        ...
        self.input_resolved = {k: v.resolve(self.findings) for k,v in self.inputs.items()}
    ...
    def get_inputs(self):
        return yaml.safe_dump(self.input_resolved, default_flow_style=False, default_style='')

    def get_outputs(self):
        return yaml.safe_dump(self.output_data, default_flow_style=False, default_style='')
